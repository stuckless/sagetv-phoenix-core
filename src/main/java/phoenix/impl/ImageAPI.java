 ...
 * reuse the cached image file.
 * <br>
 * <br>
 * Transformations are basically json (javascript object notation) structures that define one or more transformations.
 * The followsing transformations exist
 * <ul>
 ...
 * <li>{name: overlay, image: 'src/test/images/test_small.jpg', opacity:0.5, x:100, y:100}
 * <li>{name: rounded, arcSize:10}
 * <li>{name: dummy}
 * </ul>
 *
 * Because a transformation is a json structure, you can pass in an array of transformations as the transformation string. ie,
 * <b>[{name: scale, width: 100}, {name: reflection}]</b>.  If you pass in a json array as the transformation, then each transformation
 * will be applied to the buffer and then the end result will be cached to the filesystem.
 * <br>
 * <br>
 *
 * A transform can also accept an 'id' field as well.  If an 'id' is present, then that transform will automatically
 * be registered and can then be accessed by that id.
 * ie, if you used the following transform {name:scale, height: 100, id: poster_thumb}, then later you simply reference
 * that transform using {id:poster_thumb}.  This is useful if you want to register transforms for specific purposes and
 * then simply refer to them by id later in your stv code.
 * <br>
 * <br>
 * 
 * When creating a transformation, you can pass 'tag'.  A tag is simply a "grouping" which ends up being a directory
 * in the image cache.  Using a tag can be useful if you want to group together a bunch of transformations for a given
 * purpose.  You can then quickly identify them in their cached folder, and easily remove the directory if you want to
 * purge the cache.
 * 
 * @author seans
 */
...
public class ImageAPI {
    ...
    /**
     * Given an array of images, Call Sage's LoadImage on them.  
     * This will force sage to load them into it's cache.
     * 
     * This api call has been depecrated, since the UI should try to load their own images.
     * There is the potential with this API to overload the UI memory with too many images.
     * 
     * @deprecated UI Should try to load thier own images.
     *  
     * @param imageArray Array or List of images
     */
    ...
    /**
     * Given an Array/List of images, apply the same transformation to image element.  The operation happens in a background
     * thread.
     * 
     * @param imageTag common tag/name to apply to each transformation
     * @param imageArr image array
     * @param jsonTransform json transform
     * @param overwriteImages if true, it will overwrite previously cached images
     */
    ...
    /**
     * Creates a new Image by applying the given transform.  The resulting image will be cached and can get retrieved by
     * using GetImage(id, tag).
     * 
     * If overwrite is set to false, then a cached version of the image will be used.  If a cached version does not
     * exist, then it will created and cached.
     * 
     * id and tag are meant to be used together to uniquely identify a cached instance.  ie, the id may be image filename, while
     * the tag may represent the "type" of transformation that is applied.  The image tag is combined with the image id to make
     * the cached entry unique.
     * 
     * The transform is the transform, or series of transforms that you want to apply to an image using javascript notation (json).
     * ie, {name: scale, width: 100, height: 200}
     * 
     * When a tag is provided, the image cache will put all images from the same tag in the same directory.
     * This makes for easy housecleaning.  For example, if you use a tag, "poster_small" for all your
     * poster thumbnails, then you can easily remove all the poster thumbnails, by removing the poster_small dir
     * from the image cache.
     * 
     * @param id image id (ie, filename, virtual filename, or some unique id)
     * @param tag imae tag (ie, image group, poster, poster_small, background, etc) used with the image id to make the id + tag uqique
     * @param image image to transform
     * @param transform json transform to apply
     * @param overwrite true if you want to overwrite an existing cached instance
     * @return Sage image
     */
    ...
    /**
     * Same as CreateImage(id, tag, image, transform, overwrite) except that the id and tag will automatically be 
     * generated from the image and transform.
     * 
     * @param image
     * @param transform
     * @param overwrite
    ...
    /**
     * Same as CreateImage(id, tag, image, transform, overwrite) except the image id will be automatically generated
    ...
    /**
     * Gets a previously generated/cached transformation for the given image object with the specified tag.
     * 
     * For example, if a bunch of images were previously created using a tag of "poster_thumb" and a specific transform, then
     * you can recall that cached image using only the image and it's tag.
     * 
     * @param image Original image path
     * @param tag tag for the transform that was applied
     * @return
     */
    ...
    /**
     * Gets an image by it's virtual id and tag.  For example you could have created a cached transformation for a given
     * image and given it a unique id and tag.  You can then later recall the cached image using only the id and tag.  This
     * is sometimes useful if the original image is a sage image or a buffered image.  Since there isn't a file path
     * associated with the image, it could not normally be cached.  But if you gave it an id and tag, then it can be cached
     * and recalled.
     * 
     * @param id virtual id that was assigned to an image
     * @param tag arbitrary tag/group for the image
    ...
    /**
     * Registers a transformation that can be recalled later.  The transform must contains an "id" field.
     * 
     * @param transform json transformation script
     */
    ...
    /**
     * Registers a transformation that can be recalled later.  The transform is registered using the id.  This API is normally
     * used to register a composite transformation (ie json array) since there is no way to actually name the json array using
     * and id field.
     * 
     * @param transform json transformation script
     * @param id for the transformation
     */
    ...
    /**
     * Registers a transformed image that can later be recalled using the id and tag.  Register image differs from CreateImage
     * in that RegisterImage may not apply the transformation immediately.  It may queue the transformation and do it in a 
     * background thread.
     * 
     * TODO: This is not implemented... so currently RegisterImage behaves like CreateImage except that it does not return the image.
     * 
     * @param id
     * @param tag
     * @param image
    ...
    private Object cacheImageTransform(String id, String tag, Object image, String transform, boolean overwrite) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(String.format("Time: %s ms; Image: %s; Cached Image: %s; Transform: %s", et.delta(), image, f.getAbsolutePath(), transform));
        }
    ...
    private boolean isNewer(Object image, File f2) {
        if (image==null || f2==null) return false;
        File f1 = null;
    ...
    public Object CreateJustReflection(Object image, float reflectionAlphaStart, float reflectionAlphaEnd) {
        return CreateImage(image, String.format("{name: just_reflection, alphaStart: %s, alphaEnd: %s}",reflectionAlphaStart, reflectionAlphaEnd), false);
    }
    ...
    public Object CreateReflection(Object image, float reflectionAlphaStart, float reflectionAlphaEnd) {
        return CreateImage(image, String.format("{name: reflection, alphaStart: %s, alphaEnd: %s}", reflectionAlphaStart, reflectionAlphaEnd), false);
    }
    ...
    public Object CreateGradientFill(String imageId, int imgWidth, int imgHeight, float opacityStart, float opacityEnd) {
        return cacheImageTransform(imageId, String.format("gradient_%s_%s_%s_%s", imgWidth, imgHeight, opacityStart, opacityEnd), null, String.format("{name: gradient, width: %s, height: %s, opacityStart: %s, opacityEnd: %s}", imgWidth, imgHeight, opacityStart, opacityEnd), false);
    }
