...
public class MediaAPI {
	...
	private static class MetadataResolver implements
			IVariableResolver<IMediaFile> {
		private Map<String, SageProperty> props = MetadataUtil
				.getProperties(IMetadata.class);

		@Override
		public String resolve(IMediaFile file, String varName) {
			if (file == null || file.getMediaObject() == null
					|| varName == null) {
				return null;
			}
	...
	public String GetFormattedTitle(Object mediaFile) {
		...
		try {
			...
			if (mf.isType(MediaResourceType.RECORDING.value())
					|| mf.isType(MediaResourceType.EPG_AIRING.value())) {
			...
			if (mt == MediaType.MOVIE) {
				if (mf.getMetadata().getYear() <= 1800) {
					log.warn("Invalid year for Movie, so ignoring Title Mask for "
							+ mediaFile);
			...
			if (mt == MediaType.TV) {
				if (mf.getMetadata().getSeasonNumber() <= 0) {
					log.warn("Invalid Season for TV, so ignoring Title Mask for "
							+ mediaFile);
	 ...
	 * 
	 * @param mediaFile
	 * @return sage's native media object, or null if this is not a native sagetv mediafile
	 */
	public Object GetSageMediaFile(Object mediaFile) {
		...
		if (mediaFile instanceof SageMediaFile) {
			...
			return GetSageMediaFile(((ViewItem) mediaFile).getDecoratedItem());
				...
				|| sagex.api.AiringAPI.IsAiringObject(mediaFile)) {
			return mediaFile;
	...
	public File getFileSystemMediaFile(Object mediaFile) {
		...
		log.warn("Failed to get File System File for media object: "
				+ mediaFile);
		return null;
	}
	...
	/**
	 * Returns true if the media file is a Dummy node, ie, cannot be played, just informational placeholder.
	 * 
	 * @param file
	 * @return
	 */
	...
	public void ClearCustomMetadata(Object mediaFile) {
		...
		mf.accept(new ClearCustomMetadataFieldsVisitor(),
				NullProgressMonitor.INSTANCE, IMediaResource.DEEP_UNLIMITED);
	}
	...
	public ISeriesInfo GetSeriesInfo(IMediaFile mediaFile) {
		...
		// Ask sagetv for it
		seriesInfo = ShowAPI.GetShowSeriesInfo(phoenix.media
				.GetSageMediaFile(mediaFile));
		if (seriesInfo == null) {
			// still no series info, check the provider id
			id = TVSeriesUtil.createNewSeriesInfoId(md.getMediaProviderID(),
		...
		if (seriesInfo != null) {
			if (update) {
				md.setSeriesInfoID(NumberUtils.toInt(SeriesInfoAPI
	...
	/**
	 * Returns the Native File/Url, but only a path containing the last maxlen characters.
	 * 
	 * @param file
	 * @param defaultFile
	 * @param maxlen
	...
	/**
	 * Returns all child files from the given folder and it's subfolders but only for a max number of items
	 * 
	 * @param folder
	...
	/**
	 * Returns a list of all children matching the {@link MediaResourceType} for a max number of results.
	 * 
	...
	/**
	 * Returns the path relative to the root folder, whereas GetPath() return the complete path including the root folder name
	 * 
	 * @param res
	...
	public String GetPath(IMediaResource res, boolean relativeToRoot) {
		...
		if (pos==-1) return null; // is it's ourself, so we are not relative to ourself
		return path.substring(pos);
	}

	
	/**
	 * Safely gets the count of items in a Video Folder.  For online videos this will NOT force the children to be loaded,
	 * and the size will return 0 if the files have not been loaded
	 * @param folder
	 * @return
	 */
