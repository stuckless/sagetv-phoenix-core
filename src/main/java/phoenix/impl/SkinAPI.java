...
public class SkinAPI {
    ...
    /**
     * Completely removes a skin from the filesystem.  Use with care.  There is no undelete.
     * 
     * @param skin
     * @return
     */
    ...
    /**
     * Starts/Activates a skin.  NOTE, this is NOT loading the skin into the current UI context, this simply resolves the skin and
     * activates it, so that it can be loaded into the current UI context, using the LoadSkin() call.
     *  
     * @param plugin
    ...
    /**
     * Stops a skin.  ie, prevents it from being loaded using the LoadSkin() call.
     * 
     * @param plugin
     * @return
     */
    ...
    /**
     * Return a skin resource by looking for it the plugin's path or by searching it's dependencies, if it does not exist.
     * 
     * @param plugin
     * @param path
     * @return
    ...
    /**
     * Load a skin into the current context.  This will process the skin and it's image register and setup the global context for the
     * given skin.  If the Skin is not found or is not activated, then the default skin will be loaded.
    ...
    public String GetSkinProperty(String key, Object defValue) {
        ...
        if (o==null) return (defValue==null) ?null: String.valueOf(defValue);
        return o;
    }
    ...
    public List GetSkinProperties() {
        return ConfigurationUtils.filterContainsKeys("/skin/default/", Phoenix.getInstance().getConfigurationManager().getProperties(ConfigScope.USER));
    }
    ...
    public List GetSkinProperties(String startsWithFilter) {
        return ConfigurationUtils.filterContainsKeys("/skin/default/" + startsWithFilter, Phoenix.getInstance().getConfigurationManager().getProperties(ConfigScope.USER));
    }
}
