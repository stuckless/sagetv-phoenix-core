...
public class ScriptingServiceFactory {
	// if the script has an initialize method, then call it, when we load the script
    ...
    public Object callService(String client, String serviceName, Object... args) throws ScriptException, IOException, NoSuchMethodException {
    	return callService(null, client, serviceName, args);
    }

    public Object callService(String context, String client, String serviceName, Object... args) throws ScriptException, IOException, NoSuchMethodException {
        ...
        if (sp==null) {
            ...
            if (!sp.file.exists()) {
                throw new RuntimeException("Invalid Service Package: " + client + "; Missing Service File: " + sp.file.getAbsolutePath());
            }
	...
	public long getLastModifiedForScript(String client) {
        ScriptingServicePackage sp = servicePackages.get(client);
        if (sp==null) return 0;
        if (sp.file==null||!sp.file.exists()) return 0;
        return sp.file.lastModified();
	}

	/**
	 * Return an Interface instance that can map the inteface method call to the javascript method call.
	 * 
	 * @param interfaceDef
	 * @param client
	...
	public <T> T newInterfaceHandler(Class<T> interfaceDef, String client, String interfaceMethodName, String jsMethodName) {
		return (T) Proxy.newProxyInstance(interfaceDef.getClassLoader(),
                new Class<?>[] {interfaceDef},
