...
/**
 * DownloadManager is used to schedule and download remote items.  Each download item can specify how many attempts
 * to use before it will give it.  The download manager is a better alternative to just manually downloading the 
 * url contents, since it can detect errors and then reschedule the download for later.  
 * 
 * @author seans
 *
 */
public class DownloadManager {
	...
	/**
	 * Schedule a download for immediate download.  Downloads are placed in a queue and will be 
	 * distributed to one of the many download threads that are available.
	 * 
	 * @param item
	 */
	...
	/**
	 * returns the current status of the download manager.  ie, the number of running threads, waiting items, etc.
	 * @return
	 */
	...
	/**
	 * Downloads the Item and doesn't return util the item is complete.  This does not use a background thread,
	 * and it doesn not retry if the download fails.  
	 * 
	 * @param item
