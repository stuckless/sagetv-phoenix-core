...
public class WundergroundWeatherSupport implements IWeatherSupport2{
    ...
    private Integer GetCodeFromName(String IconName, IForecastPeriod.Type DayType){
        //log.info("GetCodeFromName: IconName '" + IconName + "' DayType '" + DayType + "'");
        Integer tIcon = -1;
        if (IconName==null){
            return -1;
        }else{
            if (DayType.equals(IForecastPeriod.Type.Current) || DayType.equals(IForecastPeriod.Type.Day)){ //return the Day Code
                if (CodesForDaytime.containsKey(IconName)){
                    //log.info("GetCodeFromName: returning '" + phoenix.util.ToInt(CodesForDaytime.get(IconName),-1) + "'");
                ...
                if (CodesForNighttime.containsKey(IconName)){
                    //log.info("GetCodeFromName: returning '" + phoenix.util.ToInt(CodesForNighttime.get(IconName),-1) + "'");
    ...
    public boolean update() {
        ...
        if (shouldUpdate()) {
            ...
            tCurrent.setSunrise(formatSunriseSunset(wWeather.getAstronomyInfo("SunriseHour"),wWeather.getAstronomyInfo("SunriseMinute")));
            tCurrent.setSunset(formatSunriseSunset(wWeather.getAstronomyInfo("SunsetHour"),wWeather.getAstronomyInfo("SunsetMinute")));
