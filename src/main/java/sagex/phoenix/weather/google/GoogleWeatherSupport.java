...
/**
 * Google/NWS implementation for Phoenix
 * By default all weather info is retrieved from the Google Source
 * IF the NWS service is configured, then long range forecast details will be from NWS
 *  otherwise those details that Google provides will be available
 * 
 * @author jusjoken
 */
public class GoogleWeatherSupport implements IWeatherSupport2{
    ...
    private Integer GetCodeFromURL(String ConditionURL, Type DayType){
        //log.info("GetCodeFromURL: ConditionURL '" + ConditionURL + "' DayType '" + DayType + "'");
        String Condition = "";
        ...
        if (tIcon==-1){
            ...
            if (DayType.equals(Type.Current) || DayType.equals(Type.Day)){ //return the Day Code
                if (CodesForDaytime.containsKey(Condition)){
                    //log.info("GetCodeFromURL: returning '" + phoenix.util.ToInt(CodesForDaytime.get(Condition),-1) + "'");
                ...
                if (CodesForNighttime.containsKey(Condition)){
                    //log.info("GetCodeFromURL: returning '" + phoenix.util.ToInt(CodesForNighttime.get(Condition),-1) + "'");
    ...
    public String getSourceName() {
        if (isNWSConfigured()) return "Google-NWS Weather";
        else return "Google Weather";
    }
        ...
        if (shouldUpdate()) {
            ...
            if (tWind.contains("0 mph")){
                ...
                if (WindSpeed==0){
                    ...
                    if (getUnits().equals(Units.Standard)){
                        ...
                        //Google always provides mph speeds so as Units is Metric then convert to k/h
            ...
            if (isNWS){ //handle the NWS Forecasts
                ...
                for (int i = 0; i < gWeather.getNWSPeriodCount(); i++){
                    ...
                    if (gWeather.getNWSForecastCondition(i, "tempType").equals("h")){ //h is a Day
                        ...
                        if (i==gWeather.getNWSPeriodCount()-1){
                            //last period so there is no night so add to the forcast
                            tForcast.setForecastPeriodNight(null);
                            forecast.add(tForcast);
                        }
                    }else{ //l is a Night
                        //if the first period is a night then we need to add a null Day
                        if (i==0){
                ...
                for (int i = 0; i < gWeather.getGWDayCount(); i++){
                    ...
                    tPeriod = new ForecastPeriod();
                    tPeriod.setCode(phoenix.weather2.GetCodeForceNight(GetCodeFromURL(gWeather.getGWForecastCondition(i, "iconURL"), Type.Night)));
                    tPeriod.setCondition(gWeather.getGWForecastCondition(i, "CondText"));
                    tPeriod.setDate(thisDay);
    ...
    private Date GetCurrentDate(){
        ...
        //if we get here then we did not find a match... so just return the last day we checked.
        return now.getTime();
    }
