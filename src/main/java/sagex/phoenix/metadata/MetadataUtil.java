...
public class MetadataUtil {
	 ...
	 * @throws Exception
	 */
	public static void copyMetadata(IMetadata src, IMetadata dest)
			throws Exception {
		for (Pair<Method, Method> p : MetadataUtil.getPropertyKeys().values()) {
			if (p.first() != null && p.second() != null) {
				if (p.first().getParameterTypes().length == 0) {
					log.debug("copyFrom(): No Valid Parameter Type for setter on "
							+ p.first());
				} else {
					try {
						p.first().invoke(dest, p.second().invoke(src, (Object[])null));
					} catch (Exception e) {
						log.warn("Metdata Copy Failed for Setter: " + p.first()
								+ "; Getter: " + p.second(), e);
					}
				}
			} else {
				if (p.first() == null
					...
					try {
						...
						log.warn("Metdata Copy Failed for List: " + p.second(),
								e);
					}
				} else {
					log.debug("Skipping Setter:Getter; " + p.first() + ":"
							+ p.second());
				}
			}
		}
	 ...
	 * @throws Exception
	 */
	public static void copyModifiedMetadata(IMetadata src, IMetadata dest)
			throws Exception {
		for (Map.Entry<String, Pair<Method, Method>> me : MetadataUtil
				.getPropertyKeys().entrySet()) {
			...
			if (p.first() != null && p.second() != null) {
				if (p.first().getParameterTypes().length == 0) {
					...
					if (src.isSet(getSageProperty(me.getKey()))) {
						...
						log.debug("Setting Modified Value for " + me.getKey()
								+ ": " + srcO);
				...
				if (p.first() == null
					...
					if (src.isSet(getSageProperty(me.getKey()))) {
						...
						log.debug("Skipping List Field: " + me.getKey()
								+ "; Not Set");
					}
				} else {
					log.debug("Skipping Field; " + me.getKey());
	 ...
	 * @throws Exception
	 */
	public static void mergeMetadata(IMetadata src, IMetadata dest)
			throws Exception {
		for (Pair<Method, Method> p : MetadataUtil.getPropertyKeys().values()) {
			if (p.first() != null && p.second() != null) {
				if (p.first().getParameterTypes().length == 0) {
	...
	/**
	 * Fills the metadata in Dest where the Dest is null and the source is not null.  This
	 * has the effect of updating dest from the the src, ONLY when the dest has an empty
	 * metadata field.  It does not copy from src to dest if dest has a value already.
	 * 
	 * @param src
	 * @param dest
	 * @throws Exception
	 */
	public static void fillMetadata(IMetadata src, IMetadata dest)
			throws Exception {
		for (Pair<Method, Method> p : MetadataUtil.getPropertyKeys().values()) {
			if (p.first() != null && p.second() != null) {
				if (p.first().getParameterTypes().length == 0) {
	...
	public static String getRelativePathWithTitle(IMediaFile file, IMetadata md) {
		...
		if (!file.isType(MediaResourceType.RECORDING.value())) {
			...
			if (!StringUtils.isEmpty(path)
					&& !StringUtils.isEmpty(md.getMediaTitle())) {
	...
	/**
	 * Returns true if the resource is a recorded movie, or future recorded movie
	 * 
	 * @param resource
	 * @return
	 */
	public static boolean isRecordedMovie(IMediaFile resource) {
		try {
			if (resource.isType(MediaResourceType.EPG_AIRING.value()) || resource.isType(MediaResourceType.RECORDING.value())) {
				...
				if (sagemf != null) {
					...
					if (!StringUtils.isEmpty(altCat)) {
						if (altCat.equals("Movie")
								|| altCat
										.equals(phoenix.config
												.GetProperty("alternate_movie_category"))) {
			...
			Loggers.LOG
					.warn("failed while attempting to set the media type based on alternate_movie_category",
							e);
		}
	 ...
	 * 
	 * NOTE: If you ware updating the Genre fields you can use +Genre or -Genre to add/remove a genre
	 * instead of replacing the entire genre field.
	 * 
	 * @param props
	 * @param md
	 */
	...
	/**
	 * Not a very efficient way to update metadata, but can be used in cases where you can't
	 * use the typesafe methods of {@link IMetadata}.  It is recommended to use this method
	 * any time you will use {@link IMetadata}.set() since this method will correct some values.
	 * 
	 * @param md {@link IMetadata} instance
	...
	/**
	 * Returns a metadata field from the {@link IMetadata} object.  It is recommended to use
	 * this method when calling {@link IMetadata}.get() since this method will correct some
	 * values.
	 * 
	 * @param md
	 * @param key
	...
	/**
	 * Given a non-empty rating, it will try to convert it into a valid sagetv rating.
	 * @param type
	 * @param rating
	 * @return
	 */
