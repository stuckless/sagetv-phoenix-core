...
public class TVSeriesUtil {
	...
	/**
	 * Locates a local {@link ISeriesInfo} based on an exact match of the Title and optional year.  
	 * 
	 * @param title
	 * @param year
	...
	/**
	 * Returns a {@link Pair} of String Arrays that represent the People and Characters for the given {@link ICastMember} list
	 * 
	 * @param cast
	 * @return
	 */
				...
				if (curInfo==null) {
					...
					if (info == null) {
						...
						if (prov !=null && prov instanceof ITVMetadataProvider) {
							// save this TVSeriesInfo and then link it to the mediafile
							ISeriesInfo newinfo = ((ITVMetadataProvider)prov).getSeriesInfo(newMD.getMediaProviderDataID());
							if (newinfo!=null) {
								// we have a series info, now save it
								int id = TVSeriesUtil.createNewSeriesInfoId(newMD.getMediaProviderID(), newMD.getMediaProviderDataID());
								if (id>0) {
									Pair<String[], String[]> cast = TVSeriesUtil.createCastStrings(newinfo.getCast());
									Object sageinfo = SeriesInfoAPI.AddSeriesInfo(
											id, 
											newinfo.getTitle(), 
											newinfo.getNetwork(), 
											newinfo.getDescription(), 
											newinfo.getHistory(), 
											newinfo.getPremiereDate(), 
											newinfo.getFinaleDate(), 
											newinfo.getAirDOW(), 
											newinfo.getAirHrMin(), 
											(String)newinfo.getImage(), 
											cast.first(), 
											cast.second());
									if (sageinfo!=null) {
