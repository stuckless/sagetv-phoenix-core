...
public class SearchUtil {
    ...
    /**
     * Attempts to handle dots in filename whereby if the dots are part of name, like, C.S.I or
     * S.H.I.E.L.D. then they dots remain.
     * 
     * @param val
     * @return
     */
    public static String specialHandleDots(String val) {
    	...
    	for (int i=0;i<len;i++) {
    		if (sb.charAt(i)=='.') {
    			replaceDot = true;
    			if (i+2<len && (
    					Character.isLetter(sb.charAt(i+1))
						&& (Character.isWhitespace(sb.charAt(i+2)) || sb.charAt(i+2)=='.'))
						) {
					// keep the dot because there is either another dot or space there
					replaceDot=false;
    			} else if (i-2>=0 && 
    					(Character.isLetter(val.charAt(i-1))
						&& (Character.isWhitespace(val.charAt(i-2)) || val.charAt(i-2)=='.'))) {
