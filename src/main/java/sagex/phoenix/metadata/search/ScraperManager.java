...
public abstract class ScraperManager extends SystemConfigurationFileManager implements SystemConfigurationFileManager.ConfigurationFileVisitor, IFilenameScraper {
	...
	public boolean contains(IFilenameScraper scraper) {
		for (IFilenameScraper p: scrapers) {
			if (scraper.getId().equals(p.getId())) return true;
		}
		return false;
	}
