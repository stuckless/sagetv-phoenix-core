...
/**
 * A COW (Copy On Write) Metadata proxy will proxy all requests to the Parent but it will NOT commit any writes to the parent.
 * This allows you to proxy the metadata for an object without worrying that it will physically update the parent.  If you later
 * decide that you want the parent to reflect the changes in the COW instance, you can call commit() on the object.
 * 
 * The {@link IMetadata} instance MUST be a metadata proxy instance that subclasses from {@link AbstractMetadataProxy}
 * 
 * @author seans
 *
 */
public class COWMetadataProxy extends MetadataProxy {
    ...
    public static IMetadata newInstance(IMetadata parent) {
        return (IMetadata) java.lang.reflect.Proxy.newProxyInstance(COWMetadataProxy.class.getClassLoader(), new Class[] {IMetadata.class}, new COWMetadataProxy(parent));
    }
}
