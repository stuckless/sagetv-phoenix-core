...
/**
 * Simple List of String were each String is separated by a semi-colon by default
 * 
 * @author seans
 *
 */
public class StringPropertyListFactory implements IPropertyListFactory {
	...
	public String fromList(PropertyList list) {
		...
		if (list!=null) {
			// Some lists, such as genres, for ex, can only encode a fixed # of items.
			int s = Math.min(list.size(), maxItems);
			for (int i=0;i<s;i++) {
				sb.append(encode(list.get(i)));
				if (i+1<s) sb.append(listItemSeparator);
			}
		}
