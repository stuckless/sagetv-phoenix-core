...
public class CachedUrl extends Url implements IUrl {
    ...
    public CachedUrl(String url) throws IOException {
        ...
        if (!url.toLowerCase().equals(props.getProperty("url").toLowerCase())) {
            log.error("The Cached url does not match the one passed! " + props.getProperty("url") + " != " + url
    ...
    private String getCachedFileName(String url) {
        try {
            if (url == null)
                return null;
            // now uses a simple md5 hash, which should have a fairly low
            // collision rate, especially for our
            // limited use
    ...
    public boolean isExpired(File cachedFile) {
        ...
        if (expired) {
            log.debug("CachedUrl.isExpired(): " + expired + "; File: " + cachedFile + "; LastModified: "
                    + cachedFile.lastModified() + "; Current Time: " + System.currentTimeMillis() + "; Expiry: " + secs
                    + "s; Diff: " + diff + "s");
        }
        return expired;
    }
    ...
    public static boolean isExpired(File cachedFile, long expirySecs) {
        ...
        if (expired) {
            log.debug("CachedUrl.isExpired(): " + expired + "; File: " + cachedFile + "; LastModified: "
                    + cachedFile.lastModified() + "; Current Time: " + System.currentTimeMillis() + "; Expiry: " + expirySecs
                    + "s; Diff: " + diff + "s");
        }
        return expired;
    }
    ...
    private File getCacheDir() {
        if (urlCacheDir == null) {
            urlCacheDir = new File(Phoenix.getInstance().getUserCacheDir(), "url");
    ...
    public void cache(ICookieHandler handler) throws IOException {
        ...
        URLConnection c = UrlUtil.openUrlConnection(u, followRedirects);
    ...
    public static void remove(CachedUrl url) {
        if (url != null) {
            url.remove();
        }
    }
    ...
    public String toString() {
        return "CachedUrl: " + (props != null ? props.getProperty("url") : "N/A") + "; UrlId: " + urlId + "; CachedFile: "
                + propFile;
    }
}
