...
public class PhoenixDiagnostics {
    ...
    public synchronized String report(String reportType) {
        ...
        if ("detailed".equals(reportType)) {
            try {
                ...
                reportThread.start();
                return "Report is now running.  It may take some time to complete.  Your report is located at " + f.getAbsolutePath();
            } catch (Exception e) {
    ...
    public void printReport(PrintWriter pw, boolean quick) {
        try {
            ...
            for (Enumeration e = System.getProperties().propertyNames(); e.hasMoreElements();) {
                ...
                if ("USERKEY".equals(key)) continue;
    ...
    private void printCommonReport(PrintWriter pw) {
        h1(pw, String.format("Phoenix Diagnostic Report - Date: %s", DateFormat.getDateTimeInstance().format(Calendar.getInstance().getTime())));
        ...
        if (!phoenix.api.IsAtLeastVersion(sagex.api.Version.GetVersion(), phoenix.api.GetRequiredSagexApiVersion())) {
            linef(pw, "ERROR: The required sagex.api.jar version should be %s and yours is %s", phoenix.api.GetRequiredSagexApiVersion(), sagex.api.Version.GetVersion());
        ...
        if (!phoenix.api.IsAtLeastVersion(SageTV.getSageVersion(), phoenix.api.GetRequiredSageVersion())) {
            linef(pw, "ERROR: The required SageTV version should be %s and yours is %s", phoenix.api.GetRequiredSageVersion(), SageTV.getSageVersion());
        ...
        if (fields == null) {
            ...
            line(pw, "custom_metadata_properties=MediaProviderDataID;MediaTitle;MediaType;OriginalAirDate;EpisodeTitle;EpisodeNumber;SeasonNumber;DiscNumber;UserRating");
        ...
        if (!isMDPluginEnabled) {
            ...
            line(pw, "If you want to have metadata fetched when you import new media into your library, then add the following line to your Server Sage.properties.");
            line(pw, "mediafile_metadata_parser_plugins=org.jdna.sage.MetadataUpdaterPlugin");
            nl(pw);
        }

        // test if metadata tools installed
        //line(pw, "Manual Metadata Lookups Enabled", phoenix.api.IsMetadataProviderSupportEnabled());
        ...
        if (SageFanartUtil.isParseFileTitleForTVSeriesEnabled()) {
            ...
            line(pw, "Typically this is only needed if your imported media does not have MediaTitle and MediaType.  If everything is configured correctly, and you've rescanned your media, then there should be no reason to enable this.");
            line(pw, "Enabling this can also cause a minor performance issue, since it has to run a regular expression on every media file.");
            nl(pw);
        }
        ...
        String settings[] = new String[] {
                "phoenix/mediametadata/fanartEnabled",
                "phoenix/mediametadata/fanartCentralFolder",
                "phoenix/mediametadata/parseMediaFileForTVSeries",
                "phoenix/mediametadata/tvSeriesRegex",
                "phoenix/mediametadata/tvSeasonRegex",
                "phoenix/mediametadata/fanartSupportClass",
    ...
    private void reportMediaGroup(PrintWriter pw, String title, Object[] mediaFiles) {
        ...
        line(pw, "This is a complete list of the media files in the particular category.  All Fanart about about each media item is listed.\n");
    ...
    private void printReportSingleMediaFile(PrintWriter pw, Object item) {
        ...
        if (smf.getMediaType() == MediaType.TV) {
            if (StringUtils.isEmpty(SageFanartUtil.GetMediaFileMetadata(item, "MediaTitle")) || StringUtils.isEmpty(SageFanartUtil.GetMediaFileMetadata(item, "MediaType")) || StringUtils.isEmpty(SageFanartUtil.GetMediaFileMetadata(item, "SeasonNumber"))) {
        ...
        if (smf.getMediaType() == MediaType.MOVIE) {
            if (StringUtils.isEmpty(SageFanartUtil.GetMediaFileMetadata(item, "MediaTitle")) || StringUtils.isEmpty(SageFanartUtil.GetMediaFileMetadata(item, "MediaType"))) {
