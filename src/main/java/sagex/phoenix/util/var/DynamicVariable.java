...
/**
 * A Dynamic Variable will dynamically manage one of more other Variable types by inspecting the value
 * that is being set, and determing what the best variable should be.
 * 
 * @author seans
 *
 * @param <T>
...
public class DynamicVariable<T> extends Variable<T> {
	...
	/**
	 * Sets the value of the dynamic variable.  The value can be a formatted expressions, such as the following
	 * 
	 * <pre>
	...
	public void setValue(String value) {
		...
		if (value.startsWith("${")) {
			...
			if (m.matches()) {
				...
				if (scope==null || scope.trim().length()==0 || ScopedConfigurationPropertyVariable.Scope.Client.name().equalsIgnoreCase(scope)) {
	...
	/**
	 * Returns the value of the current Variable as string.  If the value is null, then an empty string is returned.
	 * 
