...
public class SkinManager extends SystemConfigurationFileManager implements SystemConfigurationFileManager.ConfigurationFileVisitor {
    ...
    public boolean activatePlugin(ISkin p) {
        if (p.getState()==State.STARTING) {
            log.error("Attempting to activate a plugin that is already starting.  This is probably a cyclic dependency for: " + p.getId());
            p.setState(State.FAILED);
            p.setStatus("Can't Activate.  Possible cyclical dependencies.");
            return false;
        }
        
        if (p.getState()==State.ACTIVE) return true;
        if (p.getState()==State.FAILED) return false;
        if (p.getState()!=State.RESOLVED) return false;
    ...
    private boolean resolvePlugin(ISkin p) {
        ...
        if (p.getState()== State.ACTIVE) return true;
        if (p.getState() == State.FAILED) return false;
        if (p.getState() == State.RESOLVED) return true;
    ...
    private void installPlugins() {
        for (ISkin p : skins) {
            if (p.getState()==State.UNINSTALLED) {
                // TODO: Actually Install the plugin, for new we are assuming that it is already installed
    ...
    public void loadSkin(String uiContext, ISkin skin) {
        ...
        for (Enumeration e = images.propertyNames() ; e.hasMoreElements();) {
            ...
            if (imgFile == null) {
                ...
                log.info("Adding Global Skin Variable; UI: " + uiContext + "; Variable: " + var + "; Image: " + imgFile.getAbsolutePath());
