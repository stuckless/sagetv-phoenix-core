...
public class FileResourceFactory {
    ...
    // TODO: Maybe we can use this to enable BMT to use it's scraper files for testing file types
    ...
    private static Pattern videoPattern = Pattern.compile("\\.avi$|\\.mpg$|\\.divx$|\\.mkv$|\\.wmv$|\\.mov$|\\.xvid$|\\.ts$|\\.m2ts$|\\.m4v$|\\.mp4$|\\.iso$", Pattern.CASE_INSENSITIVE);
    private static Pattern tvPattern = Pattern.compile("(season\\s*[0-9]{1,2})|(s[0-9]{1,2}[\\ ex\\.])", Pattern.CASE_INSENSITIVE);
    private static Pattern recordingPattern = Pattern.compile("(([^-]+)-([^-]+)-([0-9]+)-([0-9]{1,2}))|(([^-]+)-([0-9]+)-([0-9]{1,2}))\\.", Pattern.CASE_INSENSITIVE);

    /**
     * createResource() is used as a factory for Filesystem {@link File} objects to ensure that a Filesystem {@link File}, such as a DVD
     * or bluray disc gets created as the correct type of {@link IMediaResource}.
     * 
     * {@link FileMediaFile} or {@link FileMediaFolder} should now be created directly but rather they should
     * only be created when using this factory.
     *  
     * @param 
     * @return
     */
    ...
    /**
     * Convenience method to create a Folder.  If the file is not a directory, then a directory of the parent is created.
     */
    ...
    /**
     * resolveBluRay() attempt to resolve a file as a BluRay folder.  It will return the path of the BDMV folder or a parent folder that
     * contains a .m2ts file.
     *  
     * @param f
     * @return
     */
    ...
    /**
     * resolveDVD() attempts to resolve a DVD file by inspecting the file to see if it a dvd file.  If it is a dvd file, then it will 
     * return it as a directory of either the VIDEO_TS dir or a dir that contains a .vob file.
     * 
     * @param f
     * @return
     */
    ...
    /**
     * Given a file that may be a reg file, or dvd, or bluray, return the meaninful filename.  ie, if VIDEO_TS is passed in
     * then the parent filename is returned, since VIDEO_TS is not meaninful.
     * 
     * @param f
     */
