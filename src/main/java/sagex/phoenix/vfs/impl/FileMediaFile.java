...
public class FileMediaFile extends VirtualMediaFile implements IMediaFile, Serializable {
    ...
    /**
     * File MediaFile can only be created using the {@link FileResourceFactory}, to ensure
     * that the correct type of file is created, ie, normal, dvd, bluray, etc.
     * 
     * @param file
     */
	...
	protected IMetadata createMetadata() {
		if (!SageAPI.isRemote()) {
			// if we are embedded then check if this file has a sagetv corresponding mediafile with metadata
