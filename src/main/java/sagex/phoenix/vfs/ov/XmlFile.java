	...
	public void updateIds() {
		if ("Music".equals(getMetadata().getMediaType())) {
			if (StringUtils.isEmpty(getMetadata().getMediaTitle())) {
				getMetadata().setMediaTitle(sagex.phoenix.util.StringUtils.firstNonEmpty(getMetadata().getRelativePathWithTitle(), getMetadata().getEpisodeName(), getAlbumInfo().getName()));
			}
		}
		
		// take the song title from episode name
		if (!hasTitle()) {
			setTitle(sagex.phoenix.util.StringUtils.firstNonEmpty(getMetadata().getRelativePathWithTitle(), getMetadata().getEpisodeName(), getMetadata().getMediaTitle()));
		}
