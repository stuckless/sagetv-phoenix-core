...
public class ViewsBuilder extends VFSManagerBuilder {
    ...
    public void startElement(String uri, String localName, String name, Attributes attributes) throws SAXException {
    	...
    	if (name.equals("views")) {
    		...
    		if (source == null) error("unknown source: " + XmlUtil.attr(attributes, "name"));
    		try {
				configurables.push((IConfigurable) source.clone());
			} catch (CloneNotSupportedException e) {
    		...
    		if (f==null) error("unknown grouper by: " + XmlUtil.attr(attributes, "by"));
			configurables.push(f.create(null));
    	} else if (name.equals("filter")) {
    		FilterFactory f =  manager.getVFSFilterFactory().getFactory(XmlUtil.attr(attributes, "by"));
    		if (f==null) error("unknown filter by: " + XmlUtil.attr(attributes, "by"));
    		IConfigurable configurable = f.create(null);
    		if (configurable==null) error("failed to create filter for " + XmlUtil.attr(attributes, "by"));
    		...
    		if (f==null) error("unknown sort by: " + XmlUtil.attr(attributes, "by"));
			IConfigurable configurable = f.create(null);
    ...
    private void addOption(Attributes attributes, String optName, IConfigurable configurable2) throws SAXException {
    	if (configurable2==null) error("can't add option to null configurable item");
