...
/**
 * Groups based on the value of the timestamp on the media file
 * Can group either by Year, Month, or Day depending on the option passed in
 *  
 * @author bialio
 */
public class FileDateGrouper implements IGrouper, HasOptions {
    ...
    private static final String[] strMonths = new String[]{
            "January",
            "February", 
            "March",
            "April",
    ...
    public FileDateGrouper() {
		options.add(new ConfigurableOption(OPT_METADATA_FIELD, "Date Field Name", null, DataType.string, true, ListSelection.single, ConfigList.fileDateList()));
    }
	...
	public void onUpdate(BaseConfigurable parent) {
		...
		if (dateField==null || 
		    (!dateField.equalsIgnoreCase("YEAR") && !dateField.equalsIgnoreCase("MONTH") && !dateField.equalsIgnoreCase("DAY"))) {
	...
	public String getGroupName(IMediaResource res) {
		if (dateField !=null && res instanceof IMediaFile) {
			...
			if (dateField.equalsIgnoreCase("YEAR")) {
				...
				return (strMonths[mediaCal.get(Calendar.MONTH)]);
			}
			else if (dateField.equalsIgnoreCase("DAY")) {
				return ("" + mediaCal.get(Calendar.DATE));
			}
			return ("Unknown");
		}
		return null;
