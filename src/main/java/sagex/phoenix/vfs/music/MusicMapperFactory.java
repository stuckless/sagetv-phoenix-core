...
/**
 * A Music Mapper Factory takes 2 VFS Music views and maps an offline view to files that are in your local library, 
 * it builds a new view of playable items from your library.  This is used to map a Top-## Chart view
 * to files that you already have in your Music Library.
 *  
 * @author sls
 */
public class MusicMapperFactory extends Factory<IMediaFolder> {
	...
	public MusicMapperFactory() {
		...
		addOption(new ConfigurableOption("library-view", "View name of the Libary music files", "phoenix.view.source.music", DataType.string));
		addOption(new ConfigurableOption("hide-offline", "Removes files from the view that could not be located in your library", "false", DataType.bool));
		addOption(new ConfigurableOption("cache-expiry", "Cache the view's results for the given # of minutes", "1440", DataType.integer));
	}
