...
public class ViewFactory extends Factory<ViewFolder> {
	...
	/**
	 * Flatten the view, by fetching all items. * * {@value}
	 */
	public static final String OPT_FLAT = "flat";

	/**
	 * For each source and view, don't add the source and folder, but rather the
	 * children * * {@value}
	 */
	public static final String OPT_CHILDREN = "children-only";

	/**
	 * prune single item folders option name * {@value}
	 */
	public static final String OPT_PRUNE_SINGLE_ITEM_FOLDERS = "prune-single-item-folders";

	/**
	 * Root path for the folder. For exmaple a folder may have a heirachy of
	 * /Vidoes/Movies/DVDs/ but you only want to present the view at the DVDs
	 * folder, so you set the root to /Vidoes/Movies/DVDs * * {@value}
	 */
	public static final String OPT_ROOT = "root";

	/**
	 * Bookmark path for the folder. This is similar to root in that it will
	 * attempt to find the path of the folder, but the returned value will still
	 * have a parent, so that the user can navigate up the heirarchy. When root
	 * is used, the returned view does not have a parent. * * {@value}
	 */
	...
	public ViewFactory() {
		super();
		addOption(new ConfigurableOption(OPT_FLAT, "Flatten", "true", DataType.bool, true, ListSelection.single,
				"true:Yes,false:No"));
		addOption(new ConfigurableOption(OPT_CHILDREN, "Children Only", "false", DataType.bool, true, ListSelection.single,
				"true:Yes,false:No"));
		addOption(new ConfigurableOption(OPT_PRUNE_SINGLE_ITEM_FOLDERS, "Prune Single Item Folders", null, DataType.bool, true,
				ListSelection.single, "true:Yes,false:No"));
		...
		// addOption(new ConfigurableOption(OPT_FETCH_ITEMS,
		// "Before Building View, Fetch ALL Items", null, DataType.bool, true,
		// ListSelection.single, "true:Yes,false:No"));
	}

	private void flatten(final VirtualMediaFolder parent, IMediaFolder vmf) {
		// TODO: We need to optimize this
		// This basically ensures that all the files are flattened/resolved
		// BEFORE
		// we build the view. We do this so that grouping/filtering works
		// correctly.
	...
	/**
	 * Get the View Presentation for the given View Depth or level. If there
	 * isn't a configured view, then an empty default view is returned. This
	 * method never returns null.
	 * 
	 * @param level
	 *            view level starting at 1
	...
	/**
	 * Adds a Root Filter Factory... ie a Filter that is applied to the items
	 * before they are passed to the view folder during creation.
	 * 
	 * @param filter
	 */
	...
	public String toString() {
		return "ViewFactory [" + "name: " + getName() + ", "
				+ (folderSources != null ? "folderSources=" + folderSources.size() + ", " : "")
