...
/**
 * Convenience source for configuring a base {@link XmlSourceFactory} for either RSS or Atom feeds
 * 
 * @author sls
 */
public class RSSFeedFactory extends Factory<IMediaFolder> {
	public RSSFeedFactory() {
		...
		addOption(new ConfigurableOption("mediatype", "Feed's Media Type", "Movie", DataType.string, true, ListSelection.single, ConfigList.mediaTypeList()));
		addOption(new ConfigurableOption("feedtype", "rss or atom feed", "rss", DataType.string, true, ListSelection.single, "rss:RSS,atom:Atom"));
	}
