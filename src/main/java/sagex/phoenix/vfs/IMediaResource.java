...
public interface IMediaResource extends Comparable<IMediaResource> {
	...
	/**
	 * Hint to pass the delete() api to tell the delete() method that the recording
	 * was incorrect
	 * 
	 * {@value}
	 */
	...
	/**
	 * Return the Media Object that backs this Media Resource.  Different kinds of 
	 * resources may wrap different kinds of "native" objects.  This will always return
	 * the native object that is being wrapper by the resource.
	 * @return
	 */
    public Object getMediaObject();

    /**
     * Get Title should be the title/name of the resource (for files, it would include the extension)
     * 
     * @return
     */
    ...
    /**
     * Return true if this resource "exists".  Exists may mean local on
     * the filesystem, or a valid url, if it's on only resource.
     * 
     * @return
     */
     ...
     * 
     * You can pass a Hint of HINT_DELETE_WITH_PREJUDICE=true.  It is up the implementation
     * to determine if it will handle hints, etc.
     * 
    ...
    /**
     * Sets the last modified data/time for this resource if possible. ie
     * online resources may not support this, and it should silently 
     * fail
     * @param time
     */
    ...
    /**
     * Returns a unique id for this resource.  Each resource should
    ...
    /**
     * Visit this resource using the visitor and progress monitor.  if the visitor returns true
     * and the visitor has children, then those children will be traversed, so long as it does
     * not surpass the 'deepness'.
     * 
     * At any time the visiting can be stopped by setting the monitor to cancelled.
     * 
     * An implementation should ensure that that monitor is never null.
     * 
     * Passing 0 for deep ensures that only this item is visited.  Passing 1 will ensure that this
     * item plus any child elements will be visited.
     * 
     * @param visitor
     * @param monitor
    ...
    /**
     * Returns the virtual path of this resource within the hierarchy of vfs items.  This would
     * be analogous to a file path, but this path is NOT the real path, but rather a virtual
     * path.
     * @return
     */
