...
public class DontLikeFilter extends Filter {
    ...
    public DontLikeFilter(boolean dontlike) {
    	super();
    	addOption(new ConfigurableOption(OPT_VALUE, "DontLike", "true", ConfigurableOption.DataType.string, true, ConfigurableOption.ListSelection.single, "true:Yes,false:No"));
    ...
    public boolean canAccept(IMediaResource res) {
        ...
        if (res instanceof IMediaFolder){
            //LOG.info("canAccept: IMediaFolder found so returning TRUE - OPT_VALUE = '" + dontlike + "'");
            return true;
        }
        if (res instanceof SageMediaFile) {
            //LOG.info("canAccept: SageMediaFile found so returning '" + ((SageMediaFile)res).isDontLike() == dontlike + "' - OPT_VALUE = '" + dontlike + "'");
            return ((SageMediaFile)res).isDontLike() == dontlike;
        }else if(res instanceof DecoratedMediaFile){
            //LOG.info("canAccept: DecoratedMediaFile found - OPT_VALUE = '" + dontlike + "' res '" + res + "'");
            return canAccept(((DecoratedMediaFile) res).getDecoratedItem());
        } else {
            //LOG.info("canAccept: default returning FALSE - OPT_VALUE = '" + dontlike + "' res '" + res + "'");
            return false;
        }
    }
}
