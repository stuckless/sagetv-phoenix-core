...
/**
 * Filters are used to restrict a view based on a criteria.  Subclasses must implement the
 * canAccept() method, and define their configurable options in the constructor.  You must
 * call super() from the constructor to ensure that the base filter options get added.
 * 
 * Subclasses must also extend the clone method, if they have objects that require deep
 * cloning.
 * 
 * @author seans
 */
public abstract class Filter extends BaseConfigurable implements IResourceFilter, Cloneable, HasName, HasLabel {
	...
	/**
	 * Most filters can have a "value", if they do, then they 
	 * should use this option name.
	 * 
	 * {@value}
	 */
    ...
    public Filter() {
    	super();
    	addOption(new ConfigurableOption(OPT_SCOPE, "Include/Exclude", "include", DataType.string, true, ListSelection.single, "include:Include,exclude:Exclude"));
    }
    ...
    /**
     * Subclasses can override this method to udpate thier local states when options
     * change.  This will only be called at the time that accept() method is called.
     */
	...
	/**
	 * convenience method that sets the filter's 'value' option and then
	 * updated the changed status if the value changed.
	 * 
	 * @param newValue
	 */
