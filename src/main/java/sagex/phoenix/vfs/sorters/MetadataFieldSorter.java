...
public class MetadataFieldSorter implements Comparator<IMediaResource>, Serializable, HasOptions {
    ...
    public MetadataFieldSorter() {
        options.add(new ConfigurableOption(OPT_METADATA_FIELD, "Metadata Field Name", null, DataType.string, true, ListSelection.single, ConfigList.metadataList()));
        options.add(new ConfigurableOption(OPT_COMPARE_AS_NUMBER, "Compare as Numbers", "false", DataType.bool, true, ListSelection.single, "true:Yes,no:No"));
    }
