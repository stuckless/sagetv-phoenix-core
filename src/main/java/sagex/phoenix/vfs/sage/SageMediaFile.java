...
public class SageMediaFile extends VirtualMediaFile implements IMediaFile {
    /**
     * Minimum value that you need to 'touch' a sage media file before sage knows it changed
     * {@value}
     */
    ...
    public boolean hasUserCategory(String userCategory) {
        ...
        //userCategory can be a delimited string and any are valid and checked for a match
    ...
    public void setManualRecord(boolean manual) {
    	...
    	// does nothing since you cannot change the manual recording property of a sage item
    }
    ...
    public boolean isType(int type) {
        ...
        if (rtype==MediaResourceType.FILE) {
            ...
            return isType(MediaResourceType.VIDEO.value()) || isType(MediaResourceType.TV.value()) || isType(MediaResourceType.DVD.value()) || isType(MediaResourceType.BLURAY.value());
	...
	public IMetadata createMetadata() {
		if (isType(MediaResourceType.EPG_AIRING.value())) {
			...
			// basically for non existing sage files (ie, just simple airings, then do not write directly to the file
			//return COWMetadataProxy.newInstance(MetadataUtil.createMetadata(getMediaObject()));
			return AiringMetadataProxy.newInstance(getMediaObject());
		} else {
			// creates a metadata object that has direct writes to the wiz.bin metadata
