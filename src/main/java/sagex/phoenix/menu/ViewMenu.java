    ...
    public List<IMenuItem> getItems() {
    	...
    	for (IMediaResource r: fold) {
    		if (r instanceof IMediaFolder) {
    			...
    			if (contextVar!=null) {
    				// ensures the current menu item is added to the static context, before
    ...
    private ViewFolder getFolder() {
    	if (folder==null) {
	    	...
	    	if (folder==null) {
	    		...
	    		folder = new ViewFolder(new sagex.phoenix.vfs.views.ViewFactory(), 0, null, new VirtualMediaFolder("ERROR: " + getName()));
	    	}
    	}
    	return folder;
	}
