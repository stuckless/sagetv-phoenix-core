...
/**
 * TaskManager is used to manager long running tasks that require rety abilities.  If you only need to run 
 * simple background task that does not require retries, then use the java {@link Timer} and {@link TimerTask}.
 * 
 * Subclasses will need implement the createTask() method to create tasks for their particular type.
 * 
 * @author seans
 *
 */
public class RetryTaskManager {
	...
	/**
	 * Main task execution point.  TaskItem requires {@link ITaskOperation} be set.
	 * 
	 * @param item
	 */
	public void performTask(TaskItem item) {
		if (item.getOperation()==null) throw new RuntimeException("TaskItem required an operation to perform");
		item.setRetries(0);
