 ...
 * <p/>
 * This Handler can accept a "transform" url parameter and accept and apply
 * a transformation can be consumed by {@link ImageAPI}. The result is the
 * transformation is applied to the image and the resulting image is sent
 * back. Transformations can be used for image scaling, ie,
 * /sagex/media/12323223?transform={name:scale,height:100} <br/>
 * To force an overwrite of an image, you can use user &overwrite=true. The
 * default is that once an image transform has been performed then it will
 * be cached. So, overwrite=true, is used to force a rebuild of the
 * transformation.
 */
public class FanartMediaRequestHandler implements SageMediaRequestHandler {
    ...
    public void processRequest(HttpServletRequest req, HttpServletResponse resp, Object sageMediaFile) throws Exception {
        ...
        if (path == null)
            throw new FileNotFoundException(MediaFileAPI.GetMediaTitle(sageMediaFile));
