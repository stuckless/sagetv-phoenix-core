...
public class SageFanartUtil {
	...
	public static SimpleMediaFile GetSimpleMediaFile(Object mediaObject, boolean ignoreMediaTitle) {
		...
		if (isEmpty(mf.getTitle()) || mf.getMediaType() == null) {
			if (IsDVDFile(mediaObject)) {
				...
				if (isEmpty(mf.getTitle())) {
					// this is a DVD or Blue-ray drive, pass back the folder
					// name to suppress the error and enable posters
		...
		if (log.isDebugEnabled()) {
			if (mf.getMediaType() == MediaType.TV) {
				log.debug("GetSimpleMedia(): Sage Object: " + mediaObject + "; MediaType: " + mf.getMediaType() + "; MediaTitle: "
						+ mf.getTitle() + "; Season: " + mf.getSeason());
			} else {
				log.debug("GetSimpleMedia(): Sage Object: " + mediaObject + "; MediaType: " + mf.getMediaType() + "; MediaTitle: "
						+ mf.getTitle());
			}
	...
	public static Pattern GetTVTitleScraperRegexp() {
		if (tvSeriesNameScraperRegex == null) {
			...
			tvSeriesNameScraperRegex = Pattern.compile((String) Configuration.GetProperty("phoenix/mediametadata/tvSeriesRegex",
					".*[\\\\/](.*)s([0-9]{1,2})\\s*[exd]([[0-9]]{1,2})"));
		}
	...
	public static Pattern GetTVSeasonScraperRegexp() {
		if (tvSeriesSeasonScraperRegex == null) {
			// default will parse a tv title from...
			// c:\\movies\\Bones S02 D03\\ - Show Title.mpg
			tvSeriesSeasonScraperRegex = Pattern.compile((String) Configuration.GetProperty("phoenix/mediametadata/tvSeasonRegex",
					".*[\\\\/].*s([0-9]{1,2})\\s*[exd]([[0-9]]{1,2})"));
		}
