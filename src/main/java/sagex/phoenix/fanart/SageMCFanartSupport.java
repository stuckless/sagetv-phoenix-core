...
public class SageMCFanartSupport implements IFanartSupport {
     ...
     */
    public File GetFanartArtifactForTitle(Object mediaObject, MediaType mediaType, String mediaTitle, MediaArtifactType artifactType, String artifactTitle, String centralFolder) {
    ...
    /**
     * Returns the value set in sagemc/media_background/central_folder or the SageMC default folder
     * ROOTPATH/MediaBackgrounds/ if it's not set.
     */
    public String GetFanartCentralFolder() {
        Object o = Configuration.GetProperty("sagemc/media_background/central_folder", null);
        if (o == null) {
            String sageMCHome = System.getProperty("user.dir") + File.separator + "STVs" + File.separator + "SageTV3" + File.separator + "SageMCE";
    ...
    public File[] GetFanartArtifacts(Object mediaObject, MediaType mediaType, String mediaTitle, MediaArtifactType artifactType, String artifactTitle, Map<String, String> metadata) {
        SimpleMediaFile mf = SageFanartUtil.GetSimpleMediaFile(mediaObject);
        File fa = GetFanartArtifactForTitle(mediaObject, 
                (mediaType==null)?mf.getMediaType():mediaType, 
                (mediaTitle==null)?mf.getTitle():mediaTitle, 
                artifactType, 
                artifactTitle, GetFanartCentralFolder());
        if (fa!=null) {
            return new File[] {fa};
        }
        return null;
    ...
    public File GetFanartArtifact(Object mediaObject, MediaType mediaType, String mediaTitle, MediaArtifactType artifactType, String artifactTitle, @SuppressWarnings("unused") Map<String, String> metadata) {
        if (mediaType == MediaType.ACTOR || mediaType == MediaType.GENRE || artifactType == MediaArtifactType.ACTOR) return null;
        ...
        if ((file==null || !file.exists()) && mf.getMediaType() == MediaType.MUSIC) {
            file = GetFanartArtifactForTitle(mediaObject, MediaType.MUSIC, SageFanartUtil.GetAlbumArtist(mediaObject), artifactType, artifactTitle, fanartFolder);

            if (!file.exists()) {
                file = GetFanartArtifactForTitle(mediaObject, MediaType.MUSIC, SageFanartUtil.GetAlbumPersonArtist(mediaObject), artifactType, artifactTitle, fanartFolder);
            }
        }
        return file;
    }

    public File GetFanartArtifactDir(Object mediaObject, MediaType mediaType, String mediaTitle, MediaArtifactType artifactType, String artifactTitle, Map<String, String> metadata, boolean create) {
        // not implemented
        return null;
    }

    public void SetFanartArtifact(Object mediaObject, File fanart, MediaType mediaType, String mediaTitle, MediaArtifactType artifactType, String artifactTitle, Map<String, String> metadata) {
        // not implemented
    }
}
