...
public class AdvancedFanartMediaRequestHandler {
	...
	/**
	 * if usedefault=true, then use the default poster, etc, when no fanart is found.
	 */
    ...
    public void processRequest(HttpServletRequest req, HttpServletResponse resp) {
        ...
        if ("episode".equalsIgnoreCase(req.getParameter(PARAM_ARTIFACTTYPE))) {
	        ...
	        path = getFanartArtifact(
	        		mediaFile, 
	        		req.getParameter(PARAM_MEDIATYPE), 
	        		mediaTitle, 
	        		req.getParameter(PARAM_ARTIFACTTYPE),
	        		req.getParameter(PARAM_ARTIFACT_TITLE),
	        		metadata);
        }
        ...
        if (path == null && requiresDefault(req)) {
        	...
        	if (path!=null) {
        		// TODO: Create a Temp File by loading the original, and then applying the title string,
        		// if required, and then make a new image file and use that as the path.
        	}
        }
    ...
    protected String getFanartArtifact(int mediaFile, String mediaType, String mediaTitle, String artifactType, String artifactTitle,
			Map<String, String> metadata) {
    	return phoenix.fanart.GetFanartArtifact(
        		mediaFile, 
        		mediaType, 
        		mediaTitle, 
        		artifactType,
