 ...
 * learned from looking at the original c++ code
 * http://xbmc.svn.sourceforge.net/
 * viewvc/xbmc/trunk/XBMC/xbmc/utils/IMDB.cpp?view=markup
 *
 * @author seans
 */
public class XbmcMovieProcessor {
    ...
    /**
     * As per xbmc scraper, if the nfo data has the details url, then this url
     * is the url of the details for the item.
     * <p/>
     * <pre>
     * $$1 is nfo contents
     * </pre>
    ...
    public XbmcUrl getSearchUrl(String title, String date) throws Exception {
        if (date == null)
            date = "";
        String url = scraperProcessor.executeFunction(FUNCTION_CREATE_SEARCH_URL, new String[]{"", UrlUtil.encode(title),
     ...
     * @throws Exception
     */
    public String getSearchReulsts(XbmcUrl url) throws Exception {
    ...
    private String parseIdFromUrl(String url) {
        ...
        if (StringUtils.isEmpty(movieId)) {
            try {
                Pattern p = Pattern.compile("http://www.thetvdb.com/api/1D62F2F90030C444/series/([0-9]*)/all");
                Matcher m = p.matcher(url);
                if (m.find()) {
                    movieId = m.group(1);
