...
public class ConfigurationMetadataManager extends SystemConfigurationFileManager implements SystemConfigurationFileManager.ConfigurationFileVisitor {
    ...
    public void addMetadata(Group group) throws Exception {
        ...
        if (ce != null) {
            ...
            if (group.getLabel() == null) group.setLabel(ce.getLabel());
            if (group.getDescription() == null) group.setDescription(ce.getDescription());
    ...
    /**
     * Adds a transient (ie non persistent) configuration element field to the field list.  This is
     * mainly used to add a typed field to the metadata after the system has been loaded.
     * 
	...
	public void visitConfigurationFile(ConfigurationType type, File file) {
		try {
	        ...
	        Group gr[] = prov.load();
	        if (gr==null || gr.length==0) throw new IOException("File loaded, but no metadata was included.");
            addMetadata(gr);
		} catch (Exception e) {
