...
/**
 * Manages sets of configuration files from 3 primary sources; System, Plugins, and User Contributed.  Configuration files
 * are loaded in that order to ensure that plugins can override the system, and the users can override the system or plugin.
 * 
 * Subclasses can inherit from this class to inherit the ability to have 3 classes of configuration files, that are loaded
 * in a particular order.
 * 
 * if the configuration property, 'phoenix/safeMode' is true, then ONLY system configurations are loaded.
 * 
 * @author sean
 */
public abstract class SystemConfigurationFileManager implements HasFileConfigurations {
	public enum ConfigurationType {System, User, Plugin};
