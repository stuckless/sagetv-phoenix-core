...
public class TestSearchQuery {
	...
	public void testFileMatcherBuilder() {
		FileMatcherManager mgr = new FileMatcherManager(new File("src/test/java/test/junit"), new File("target/testing/userdata/Phoenix/scrapers"));
	...
	public void testMovieMatchers() {
		IMediaFile mf = (IMediaFile) FileResourceFactory.createResource(new File("target/junit/testing/Time.Traveller.1967.dvdrip.avi"));
		SearchQuery q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		...
		mf = (IMediaFile) FileResourceFactory.createResource(new File("target/junit/testing/Total Recall 2010 1998 DvdRip 720p.avi"));
		q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		...
		q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		verify(q, MediaType.MOVIE, Field.RAW_TITLE, "I'm OK & That's The Truth", Field.CLEAN_TITLE, "I'm OK & That's The Truth", Field.YEAR, "2010");
		
		mf = (IMediaFile) FileResourceFactory.createResource(new File("target/junit/testing/Savages.2012.720p.bluray.x264-sparks.mkv"));
		q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
	...
	public void testTVMatchers() {
		...
		SearchQuery q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		verify(q, MediaType.TV, Field.RAW_TITLE, "Lost Again", Field.CLEAN_TITLE,  "Lost Again", Field.SEASON, "01", Field.EPISODE, "02");

		mf = (IMediaFile) FileResourceFactory.createResource(new File("target/junit/testing/BigBangTheory-NewEpisode-000000-0.avi"));
		q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		verify(q, MediaType.TV, Field.RAW_TITLE, "Big Bang Theory", Field.CLEAN_TITLE,  "Big Bang Theory", Field.EPISODE_TITLE, "New Episode");
		...
		q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		verify(q, MediaType.TV, Field.RAW_TITLE, "Series Title", Field.CLEAN_TITLE,  "Series Title", Field.EPISODE_TITLE, "Episode Title");
		...
		mf = (IMediaFile) FileResourceFactory.createResource(new File("target/junit/testing/The X-Files-S06E20-three of a kind.avi"));
		q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		verify(q, MediaType.TV, Field.RAW_TITLE, "The X-Files", Field.CLEAN_TITLE,  "The X-Files", Field.EPISODE, "20", Field.SEASON, "06");

		mf = (IMediaFile) FileResourceFactory.createResource(new File("target/junit/testing/TheMiddle-S04E03-TheSecondAct-6260432-0.mp4"));
		q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		verify(q, MediaType.TV, Field.RAW_TITLE, "The Middle", Field.CLEAN_TITLE,  "The Middle", Field.EPISODE, "03", Field.SEASON, "04");

		mf = (IMediaFile) FileResourceFactory.createResource(new File("target/junit/testing/Marvel's Agents of S.H.I.E.L.D. - S01E01 - Pilot [WEBDL-1080p].mkv"));
		q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		verify(q, MediaType.TV, Field.RAW_TITLE, "Marvel's Agents of S.H.I.E.L.D", Field.CLEAN_TITLE,  "Marvel's Agents of S.H.I.E.L.D", Field.EPISODE, "01", Field.SEASON, "01");
	}
	
	@Test
	public void testTVWithYears() throws Exception {
		IMediaFile mf = (IMediaFile) FileResourceFactory.createResource(new File("target/junit/testing/Archer (2009)/Season 01/Archer (2009) - S01E01 - Mole Hunt.mkv"));
		SearchQuery q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		verify(q, MediaType.TV, Field.RAW_TITLE, "Archer (2009)", Field.CLEAN_TITLE,  "Archer", Field.SEASON, "01", Field.EPISODE, "01");

		verify(q, MediaType.TV, Field.RAW_TITLE, "Archer (2009)", Field.CLEAN_TITLE,  "Archer", Field.SEASON, "01", Field.EPISODE, "01", Field.YEAR, null);
		...
		q = Phoenix.getInstance().getSearchQueryFactory().createQueryFromFilename(mf, new Hints());
		verify(q, MediaType.TV, Field.RAW_TITLE, "Archer", Field.CLEAN_TITLE,  "Archer", Field.SEASON, "01", Field.EPISODE, "01", Field.PROVIDER, "tvdb", Field.ID, "110381", Field.YEAR, "2009");
	}
	
	@Test
	public void TestAiringQuery() {
		SimpleStubAPI api = new SimpleStubAPI();
		api.overrideAPI("GetMediaFileID",0); // should always return 0, since we are dealing with Airings 
		...
		a.put("GetOriginalAiringDate", Calendar.getInstance().getTime().getTime()); // populate API
		...
		SearchQuery q = Phoenix.getInstance().getSearchQueryFactory().createSageFriendlyQuery(smf, Phoenix.getInstance().getMetadataManager().getDefaultMetadataOptions());
		verify(q, MediaType.MOVIE, Field.RAW_TITLE, "The Terminator", Field.CLEAN_TITLE,  "The Terminator", Field.YEAR, "1994");
		
		// test that the type is a movie if the genre is a not a move but the show id starts with mv
		q = Phoenix.getInstance().getSearchQueryFactory().createSageFriendlyQuery(smf, Phoenix.getInstance().getMetadataManager().getDefaultMetadataOptions());
		verify(q, MediaType.MOVIE, Field.RAW_TITLE, "The Terminator", Field.CLEAN_TITLE,  "The Terminator", Field.YEAR, "1994");

		// test that the type is a tv if we can't determine type from categor or show id
