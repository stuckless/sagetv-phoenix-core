...
public class TestImageAPI {
    ...
    public void testCreateImage() throws Exception {
        ...
        // run it again, and this time we should have a new image (overwrite=true)
        image = phoenix.image.CreateImage("myimage", "test", src, "{name: scale, width: 150}", true);
        f = ImageUtil.getCachedImageFile("myimage", "test", "jpg");
        verifyImageSize(image, 150, 600);

        
        // touch the image file and run the test again, this time it should be updated
        ...
        // another transformation using the same file and tag, but different transform
    ...
    public void testCachedImages() {
        ...
        assertEquals("incorrect cached filename", DigestUtils.md5Hex("id" + "_" + "tag"), f.getName().substring(0, f.getName().indexOf(".")));
    }
