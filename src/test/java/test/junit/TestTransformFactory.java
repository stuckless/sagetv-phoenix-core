...
public class TestTransformFactory {
    ...
    public void testCompositeTransform() throws Exception {
        verifyImageSize(factory.applyTransform(createBufferedImage(400, 800), "[{name: scale, width:100},{name: rotate, theta: 90}]"), 200, 100);
    }
    
    @Test
    public void testGradientTransform() throws Exception {
        verifyImageSize(factory.applyTransform(null, "{name: gradient, width: 500, height: 600, opacityStart:0, opacityEnd:1}"), 500, 600);
    }
    ...
    public void testOverlayTransform() throws Exception {
        BufferedImage bi = ImageUtil.readImage(new File("src/test/images/test.png"));
        IBufferedTransform bt = factory.createTransform("{name: overlay, image: 'src/test/images/test_small.png', opacity:.5, x:100, y:100}");
